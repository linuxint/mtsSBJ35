...
DROP TABLE COM_MENU CASCADE CONSTRAINTS;;
...
-- CREATE TABLE TBL_BUCKET (
-- 	BKT_SEQ NUMBER(10) NOT NULL,
-- 	BKT_TITLE VARCHAR2(100) NOT NULL,
-- 	BKT_CONTENT VARCHAR2(4000),
...
-- ========================================
-- 표준단어 목록 (Standard Vocabulary)
-- ========================================

/*
표준단어 정의 규칙:
1. 글자수 제한: 최대 4글자 (약어 사용)
2. 대문자 사용: 모든 표준단어는 대문자
3. 의미 명확성: 업무에서 통용되는 용어 우선
4. 확장성: 새로운 용어 추가 시 기존 규칙 준수
5. 우선순위: 2글자 > 3글자 > 4글자 (짧은 것 우선)
...
CN - 연결 (Connection)
BK - 버킷 (Bucket)
BL - 바이너리 (Binary Large Object)
MD - 수정 (Modify)

=== 3글자 표준단어 (2글자로 표현 불가능한 경우) ===
FLG - 플래그 (Flag)
TYP - 유형 (Type)
STS - 상태 (Status)
TTL - 제목 (Title)
...
BKT - 버킷 (Bucket)
STT - 상태 (State)
BLB - 바이너리 (Binary Large Object)
SQC - 순번 (Sequence)
MOD - 수정 (Modify)
BKT - 버킷 (Bucket)
CNT - 내용 (Content)

=== 4글자 표준단어 (3글자로 표현 불가능한 경우) ===
FLAG - 플래그 (Flag)
TYPE - 유형 (Type)
STAT - 상태 (Status)
TITL - 제목 (Title)
...
4. 언더스코어(_) 사용 지양
5. 대문자 사용 권장
6. 글자수 제한: 표준단어 최대 4글자
7. 우선순위: 2글자 > 3글자 > 4글자 (짧은 것 우선)

명명 규칙 예시:
- 사용자번호: USERNO (USER + NO)
- 사용자명: USERNM (USER + NM)
- 등록일자: REGDT (REG + DT)
- 수정일자: CHGDT (CHG + DT)
- 삭제여부: DELFLG (DEL + FLG)
