...
CREATE TABLE PRJ_PROJECT (
    PRNO        BIGINT,
    PRSTARTDATE VARCHAR(10),
    PRENDDATE   VARCHAR(10),
    PRTITLE     VARCHAR(100),
...
CREATE TABLE PRJ_TASK (
    PRNO        BIGINT,
    TSNO        BIGINT NOT NULL,
    TSPARENT    BIGINT,
    TSSORT      BIGINT,
    TSTITLE     VARCHAR(100),
    TSSTARTDATE VARCHAR(10),
    TSENDDATE   VARCHAR(10),
    TSENDREAL   VARCHAR(10),
    TSRATE      BIGINT,
    OLDNO       BIGINT,
    DELETEFLAG  VARCHAR(1) DEFAULT 'N',
...
CREATE TABLE COM_CODE_V2 (
    CODECD VARCHAR(5) NOT NULL constraint COM_CODE_V2_PK primary key,
    PCODECD VARCHAR(5) constraint COM_CODE_V2_FK references COM_CODE_V2,
    CODENM     VARCHAR(40),
...
CREATE TABLE TBL_BOARD (
    BGNO       BIGINT       DEFAULT 0,
    BRDNO      BIGINT NOT NULL,
...
CREATE TABLE TBL_BOARDREPLY (
    BRDNO        BIGINT NOT NULL,
    RENO         BIGINT NOT NULL,
    USERNO       BIGINT,
    REMEMO       VARCHAR(500) DEFAULT NULL,
    REPARENT     BIGINT       DEFAULT 0,
    REDEPTH      BIGINT,
    REORDER      BIGINT,
    REGDATE      TIMESTAMP(6) DEFAULT CURRENT_TIMESTAMP,
    REDELETEFLAG VARCHAR(1)   DEFAULT 'N',
    CHGDATE      TIMESTAMP(6) DEFAULT CURRENT_TIMESTAMP,
    CHGUSERNO    BIGINT,
...
CREATE TABLE TBL_BOARDGROUP (
    BGNO       BIGINT NOT NULL,
    BGNAME     VARCHAR(50),
    BGPARENT   BIGINT       DEFAULT 0,
    DELETEFLAG VARCHAR(1)   DEFAULT 'N',
    BGUSED     CHAR(1),
    BGREPLY    CHAR(1),
    BGREADONLY CHAR(1),
...
ti < 60 * 24 * 7 * 4 THEN
    ...
    ELSIF
MONTHS_BETWEEN(SYSDATE, dt_) < 1 THEN
        ret_ := '지난달';
    ELSIF
MONTHS_BETWEEN(SYSDATE, dt_) < 13 THEN
...
END IF;
    ...
    DBMS_OUTPUT.PUT_LINE
('sdate=' || TO_CHAR(sdate, 'YYYY-MM-DD'));
    DBMS_OUTPUT.PUT_LINE
('edate=' || TO_CHAR(edate, 'YYYY-MM-DD'));
...
tsrate_ < 100 THEN
        IF tsstartdate > today_ THEN
            bgcolor_ := 'gray';
        ELSIF
tsenddate < today_ THEN
            bgcolor_ := 'red';
        ELSIF
(today_ - tsstartdate) < (tsenddate - today_) THEN
            bgcolor_ := 'green';
        ELSIF
(today_ - tsstartdate) >= (tsenddate - today_) THEN
            bgcolor_ := 'yellow';
