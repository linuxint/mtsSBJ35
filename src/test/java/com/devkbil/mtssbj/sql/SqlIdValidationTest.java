...
public class SqlIdValidationTest {
    ...
    void testSqlIdsMatch() {
        ...
        List<String> serviceSqlIds = ServiceSqlIdExtractor.extractSqlIds(
            "src/main/java/com/devkbil/mtssbj"); // Service 파일에서 SQL ID 추출

        // 3. 결과 저장 (병렬 검증)
        // SQL ID가 올바른지 Namespace 및 XML의 SQL ID와 비교
        Map<String, String> resultMap = verifySqlIds(serviceSqlIds, namespaces, sqlXmlIdsWithoutNamespace);
    ...
    private Map<String, String> verifySqlIds(
        List<String> serviceSqlIds, Set<String> namespaces, Set<String> xmlSqlIdsWithoutNamespace) {
                ...
                batch -> batch.forEach(
                    sqlId -> {
                        if (sqlId.contains(".")) { // SQL ID에 Namespace가 포함된 경우
                            ...
                            } else if (!xmlSqlIdsWithoutNamespace.contains(
                                pureSqlId)) { // SQL ID가 없으면 실패 처리
                            ...
                            if (!xmlSqlIdsWithoutNamespace.contains(sqlId)) {
                                resultMap.put(
                                    sqlId,
                                    ANSI_RED + " 실패 (SQL ID 없음)" + ANSI_RESET); // SQL ID가 없으면 실패 처리
                            } else {
                                resultMap.put(sqlId, "성공"); // 검증 성공
                            }
                        }
    ...
    private void printResults(Map<String, String> resultMap) {
        ...
        resultMap.forEach(
            (sqlId, result) -> log.info("SQL ID: {} -> 결과: {}", sqlId, result)); // 모든 결과 출력
        log.info("=========================");
